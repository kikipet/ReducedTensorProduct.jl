...
import torch
from e3nn import o3
from e3nn.o3._reduce import orthonormalize
from e3nn.math._reduce import germinate_formulas, reduce_permutation
    ...
    for ir_left, path_left, C_left in _wigner_nj(*irrepss_left, normalization=normalization, filter_ir_mid=filter_ir_mid, dtype=dtype, device=device):
        ...
        for mul, ir in irreps_right:
            for ir_out in ir_left * ir:
                ...
                triple = (ir_out.l, ir_left.l, ir.l)
...
class ReducedTensorProducts(CodeGenMixin, torch.nn.Module):
    ...
    **kwargs : dict of `e3nn.o3.Irreps`
    ...
    def __init__(self, formula, filter_ir_out=None, filter_ir_mid=None, eps=1e-9, **irreps):
        ...
        ) # for "ij=-ji", base_perm has dimensions 3x3x3
        ...
        P = base_perm.flatten(1)  # [permutation basis, input basis] (a,omega)    # for "ij=-ji", P has dimensions 3x9
        PP = P @ P.T  # (a,a)
